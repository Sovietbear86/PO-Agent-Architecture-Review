  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Extract workflow_status.name from swtr_attributes for display */}
        {(() => {
          let workflowStatusName = undefined;
          if (task.sourceData?.swtr_attributes) {
            ...
            if (statusAttr && statusAttr.value && typeof statusAttr.value === 'object') {
              workflowStatusName = statusAttr.value.name;
            }
          }
          return (
            <TaskStatusBadge
      ...
      <div style={{
        ...
        {/* Show created date if available, otherwise show updated date */}
        {task.createdAt || task.created_at ? (
          <span>
            📅 {new Date(getDateValue(task.createdAt, 'created_at')!).toLocaleDateString('ru-RU')}
          </span>
        ) : task.updatedAt || task.updated_at ? (
          <span>
            📅 {new Date(getDateValue(task.updatedAt, 'updated_at')!).toLocaleDateString('ru-RU')}
          </span>
        ) : null}
      </div>
    </div>
