...
4. Temporary `/tmp` scripts are allowed only for isolated in-memory candidate evaluation / isolated SkillRegistry lifecycle checks and must not be committed.
...
`TARGET_BRANCH = feat/learning-loop-014-v1`

`ASSIGNMENT_PATH = qa_assignments/LEARNING_LOOP_014_SPRINT_HEALTH_ROLLBACK.md`

`REPORT_PATH = qa_reports/LEARNING_LOOP_014_SPRINT_HEALTH_ROLLBACK.md`

Read first:
- `PO_AGENT_HARNESS_EVOLUTION_PLAN.md`
- `PO_AGENT_HARNESS_EVOLUTION_PLAN_STATUS_013.md`
- `PO_AGENT_HARNESS_EVOLUTION_PLAN_STATUS_014.md`
- `qa_reports/LEARNING_LOOP_013_TASK_SEARCH_SELF_IMPROVEMENT.md`
- `qa_assignments/LEARNING_LOOP_014_SPRINT_HEALTH_ROLLBACK.md`

Then execute the assignment exactly. Do not start Gate D. After publishing the report, stop and tell the user only that the report has been published.
