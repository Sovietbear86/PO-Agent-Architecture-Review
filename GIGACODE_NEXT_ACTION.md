...
The active assignment is **045**, not 006, 017, 026, 029, 030, 031, 032, 033, 034, 035, 036, 037, 038, 039, 040, 041, 042, 043, 044 or any other historical assignment.

Assignment 044 is accepted as a valid BLOCKED report. It proved no HTTP 500 and no internal `KeyError` in owner smoke tests, but it also showed that runtime identity and Task API route contract must be proven before oracle can resume.
...
5. Read `qa_assignments/CORE8_RUNTIME_IDENTITY_AND_SWTR_READ_ROUTE_RETEST_045.md` completely.
6. Verify all three values below. If any value differs, do not run another assignment and do not modify any historical report; create the expected 045 report with `045_VERDICT = BLOCKED`, exact mismatch evidence, then stop.

```text
ACTIVE_ASSIGNMENT = 045
ACTIVE_ASSIGNMENT_FILE = qa_assignments/CORE8_RUNTIME_IDENTITY_AND_SWTR_READ_ROUTE_RETEST_045.md
ALLOWED_REPORT_FILE = qa_reports/CORE8_RUNTIME_IDENTITY_AND_SWTR_READ_ROUTE_RETEST_045.md
```
...
- Do not run full tenant-wide task sync for 045.
...
The staged file list must contain exactly the allowed 045 report. If any other path appears, do not commit until it is unstaged. Never modify or stage a historical report or result.
