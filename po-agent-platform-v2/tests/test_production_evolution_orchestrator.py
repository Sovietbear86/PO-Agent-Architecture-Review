...
def test_signed_approval_is_required_before_promotion():
    ...
    with pytest.raises(ValueError, match="approved"):
        orchestrator.promote(
            session.session_id,
            approval=SimpleNamespace(approval_id="fake"),
...
def test_full_governed_path_can_enter_monitor_and_rollback_to_known_good():
    orchestrator, lifecycle, governance = _build(("c1", "c2"))

    # Create an earlier governed release to serve as the known-good rollback target.
    ...
    known_good_approval = governance.issue_human_approval(
        binding=known_good_binding,
        approved_by="owner",
    )
    ...
    approval = orchestrator.request_human_approval(session.session_id, approved_by="owner")
    manifest = orchestrator.promote(session.session_id, approval=approval, release_ref="release-1")
    monitoring = orchestrator.begin_monitoring(session.session_id)
    assert monitoring.stage is ProductionEvolutionStage.MONITOR
