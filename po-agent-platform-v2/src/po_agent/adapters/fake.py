...
class FakeAS21Adapter(AS21Adapter):
    ...
    def _init_fixtures(self) -> None:
        now = datetime.now()
        task1 = Task(key="WMB-101",id="task-001",title="Implement user authentication",description="Add OAuth2 support for user login",status=TaskStatus.RESOLVED,status_category=StatusCategory.COMPLETED_PENDING,status_transitions=[StatusTransition(from_status=TaskStatus.OPEN,to_status=TaskStatus.IN_PROGRESS,timestamp=now-timedelta(days=10),author="Ivanov.I.I"),StatusTransition(from_status=TaskStatus.IN_PROGRESS,to_status=TaskStatus.RESOLVED,timestamp=now-timedelta(days=3),author="Petrov.P.P")],created_at=now-timedelta(days=12),updated_at=now-timedelta(days=3),assignee="Ivanov.I.I",priority=TaskPriority.HIGH,source="test")
        task2 = Task(key="WMB-102",id="task-002",title="Fix login bug",description="Users cannot log in on mobile devices",status=TaskStatus.IN_PROGRESS,status_category=StatusCategory.ACTIVE_WORK,status_transitions=[StatusTransition(from_status=TaskStatus.OPEN,to_status=TaskStatus.IN_PROGRESS,timestamp=now-timedelta(days=5),author="Sidorov.S.S")],created_at=now-timedelta(days=7),updated_at=now-timedelta(days=2),assignee="Sidorov.S.S",priority=TaskPriority.CRITICAL,source="test")
        task3 = Task(key="WMB-103",id="task-003",title="Add analytics dashboard",description="Create dashboard for team metrics",status=TaskStatus.OPEN,status_category=StatusCategory.BACKLOG,created_at=now-timedelta(days=20),updated_at=now-timedelta(days=20),priority=TaskPriority.LOW,source="test")
        task4 = Task(key="DMS-201",id="task-004",title="Data migration script",description="Migrate old data to new schema",status=TaskStatus.CLOSED,status_category=StatusCategory.COMPLETED,status_transitions=[StatusTransition(from_status=TaskStatus.OPEN,to_status=TaskStatus.IN_PROGRESS,timestamp=now-timedelta(days=15)),StatusTransition(from_status=TaskStatus.IN_PROGRESS,to_status=TaskStatus.CLOSED,timestamp=now-timedelta(days=5))],created_at=now-timedelta(days=18),updated_at=now-timedelta(days=5),source="test")
        task5 = Task(key="DMS-202",id="task-005",title="API endpoint for reports",description="Create REST endpoint for report generation",status=TaskStatus.NEED_INFO,status_category=StatusCategory.WAITING,created_at=now-timedelta(days=8),updated_at=now-timedelta(days=1),source="test")
