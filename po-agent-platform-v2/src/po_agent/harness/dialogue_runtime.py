...
"""
    ...
    async def interpret(self, query: str, *, context: dict[str, Any] | None = None) -> SemanticFrame:
        ...
        response = await self.client.complete([LLMMessage(role="system", content=self.SYSTEM), LLMMessage(role="user", content=payload)], model=self.model, temperature=0.0, max_tokens=900)
        ...
        try:
            ...
            confidence = 0.0
        return SemanticFrame(data["canonical_query"].strip() or query, str(data.get("intent_hint")) if data.get("intent_hint") else None, {str(k): str(v) for k, v in (data.get("slots") or {}).items() if v is not None}, needs, max(0.0, min(1.0, confidence)), True)
...
class DialogueHarnessRuntime:
    def __init__(self, inner, interpreter: SemanticInterpreter | None = None, semantics: LearnedSemanticsStore | None = None, grounder: SemanticGrounder | None = None) -> None:
        self.inner = inner; self.interpreter = interpreter or ConservativeSemanticInterpreter(); self.semantics = semantics; self.grounder = grounder; self._pending = {}
        for name in ("adapter", "router", "capabilities", "skills"): setattr(self, name, getattr(inner, name))
    ...
    def _apply_answers(frame, answers):
        query=frame.canonical_query; slots=dict(frame.slots)
        for field,value in answers.items():
            slots[field]=value; token="{"+field+"}"
            if token in query: query=query.replace(token,value)
            elif value and value.casefold() not in query.casefold(): query=f"{query} {value}"
    ...
    def learn_explicit_definition(self, *, term, meaning, trace_id, scope="global"):
        if self.semantics is None: raise RuntimeError("learned semantics store is not configured")
    ...
    _REQUIRED_ARGS_BY_CAPABILITY={"task.lookup":("task_key",),"task.summary":("task_key",),"task.quality":("task_key",),"task.missing_requirements":("task_key",),"task.acceptance_analysis":("task_key",),"task.dependencies":("task_key",),"task.blockers":("task_key",),"task.similar":("task_key",),"task.history":("task_key",),"task.time_in_status":("task_key",),"sprint.health":("sprint_id",),"sprint.velocity":("sprint_id",),"sprint.throughput":("sprint_id",),"sprint.wip":("sprint_id",),"release.health":("release_id",),"release.scope":("release_id",),"release.progress":("release_id",),"release.blockers":("release_id",),"release.dependencies":("release_id",),"release.risk_queue":("release_id",),"team.competency_match":("task_key",),"team.assignee_recommendation":("task_key",)}

    @staticmethod
    def _build_capability_args(frame):
        slots={str(k):str(v) for k,v in frame.slots.items() if v not in (None,"")}; args={k:v for k,v in slots.items() if k in DialogueHarnessRuntime._EXECUTION_SLOT_KEYS}
        if "task_key" not in args and slots.get("task_id"): args["task_key"]=slots["task_id"]
        if "task_key" not in args and slots.get("issue_key"): args["task_key"]=slots["issue_key"]
        if "assignee" not in args and slots.get("member_login"): args["assignee"]=slots["member_login"]
        return args
    ...
    def _missing_required_source_fact(self,query):
        required_fact=getattr(self.inner,"_required_fact",None); source_facts=getattr(self.inner,"source_facts",None)
        if not callable(required_fact) or source_facts is None:return None
        required=required_fact(query); return required if required and required not in source_facts else None
    ...
    async def _execute_frame(self,frame,session,started):
        ...
        if hint=="learn_semantic":
            if self.semantics is None:return self._source_failure(session,"learning_store_unavailable","Хранилище обучаемой конфигурации недоступно.",started)
            term=(frame.slots.get("learn_term") or "").strip(); meaning=(frame.slots.get("learn_meaning") or "").strip(); scope=(frame.slots.get("learn_scope") or "global").strip()
            if not term or not meaning:
                pending=_PendingDialogue(frame,[ClarificationNeed("learn_meaning","Какое точное правило вы хотите запомнить?")]); self._pending[session]=pending; return self._clarification_response(session,pending)
            trace=str(uuid.uuid4()); rule=self.semantics.learn_explicit_definition(term=term,meaning=meaning,source_trace_id=trace,scope=scope)
            response=HarnessResponse(status=ResponseStatus.COMPLETED,trace_id=trace,session_id=session,answer=f"Запомнил правило «{rule.term}» = «{rule.meaning}»." if rule.status=="active" else "Новое правило конфликтует с уже активным. Я сохранил его как candidate и не изменил текущее поведение.",data={"learning_rule":{"id":rule.rule_id,"term":rule.term,"meaning":rule.meaning,"scope":rule.scope,"version":rule.version,"status":rule.status}},warnings=[] if rule.status=="active" else ["learning_conflict_pending"],latency_ms=(time.perf_counter()-started)*1000); self._decorate(response,frame.llm_used); return response
        if hint=="":
            response=await self.inner.process(HarnessRequest(query=frame.canonical_query,session_id=session)); self._decorate(response,frame.llm_used); return response
        skill_id=intent_to_skill_id(hint)
        if skill_id is None:return HarnessResponse(status=ResponseStatus.FAILED,trace_id=str(uuid.uuid4()),session_id=session,answer="Интент не распознан или нереализован.",intent=hint,warnings=["unsupported_semantic_intent"],latency_ms=(time.perf_counter()-started)*1000)
        capability_args=self._build_capability_args(frame); refined=self._refine_skill_id_by_slots(skill_id,frame.slots)
        try: skill=self.skills.resolve_by_id(refined)
        except ValueError:return HarnessResponse(status=ResponseStatus.FAILED,trace_id=str(uuid.uuid4()),session_id=session,answer="Навык не найден или недоступен.",intent=hint,skill_id=refined,warnings=["semantic_skill_unavailable"],latency_ms=(time.perf_counter()-started)*1000)
        valid,error=self._validate_required_args(skill.capability_id,capability_args)
        if not valid:return HarnessResponse(status=ResponseStatus.NEEDS_CLARIFICATION,trace_id=str(uuid.uuid4()),session_id=session,question=f"Мне не хватает информации: {error}.",warnings=["semantic_slot_missing"],latency_ms=(time.perf_counter()-started)*1000)
        try:
            if skill_id=="task-search" and sum(1 for k in ["assignee","sprint_id","release_id","status","product"] if k in capability_args)>=2: result=await self.capabilities.execute("task.search.composite",capability_args)
            else: result=await self.capabilities.execute(skill.capability_id,capability_args)
            response=HarnessResponse(status=ResponseStatus.COMPLETED,trace_id=str(uuid.uuid4()),session_id=session,answer=result.answer,intent=hint,skill_id=skill.id,skill_version=skill.version,data=result.data,evidence=result.evidence,warnings=result.warnings,latency_ms=(time.perf_counter()-started)*1000); self._decorate(response,frame.llm_used); return response
        except AS21CapabilityUnavailable:return self._source_failure(session,"source_capability_unavailable","Источник AS21 не предоставляет данные, необходимые для этого запроса.",started)
        except AS21SourceUnavailable:return self._source_failure(session,"source_unavailable","Источник AS21 временно недоступен. Данные не интерпретируются как пустой результат.",started)
        except AS21SourceError:return self._source_failure(session,"source_protocol_error","Источник AS21 вернул некорректные данные.",started)

    @staticmethod
    def _refine_skill_id_by_slots(skill_id,slots): return _refine_skill_id_by_slots(skill_id,slots)

    async def process(self,request):
        session=request.session_id or str(uuid.uuid4()); started=time.perf_counter()
        if not request.query or not request.query.strip():return HarnessResponse(status=ResponseStatus.FAILED,trace_id=str(uuid.uuid4()),session_id=session,answer="Запрос пуст. Пожалуйста, уточните, что вы хотите получить.",data=None,warnings=["query_empty"],latency_ms=(time.perf_counter()-started)*1000)
        if session in self._pending:
            pending=self._pending[session]; need=pending.remaining.pop(0); answer=request.query.strip()
            if not answer: pending.remaining.insert(0,need); return self._clarification_response(session,pending)
            pending.answers[need.field]=answer; pending.frame.slots[need.field]=answer
            if pending.remaining:return self._clarification_response(session,pending)
            self._pending.pop(session,None); return await self._execute_frame(self._apply_answers(pending.frame,pending.answers),session,started)
        missing_fact=self._missing_required_source_fact(request.query)
        if missing_fact:return self._source_failure(session,"source_capability_unavailable",f"Источник AS21 не предоставляет обязательные данные для этого запроса: {missing_fact}.",started,data={"missing_source_fact":missing_fact})
        semantic_context={"session_id":session}
        if self.semantics is not None: semantic_context["learned_semantics"]=self.semantics.context("global")
        if self.grounder is not None:
            try: semantic_context.update(await self.grounder.semantic_context())
            except AS21CapabilityUnavailable:return self._source_failure(session,"source_capability_unavailable","Источник AS21 не предоставляет данные для проверки контекста запроса.",started)
            except AS21SourceUnavailable:return self._source_failure(session,"source_unavailable","Источник AS21 временно недоступен. Нельзя безопасно интерпретировать запрос без проверки источника.",started)
        ...
        if self.grounder is not None and (frame.intent_hint or "").strip().casefold()!="learn_semantic":
            ...
            except AS21CapabilityUnavailable:return self._source_failure(session,"source_capability_unavailable","Источник AS21 не предоставляет данные для проверки сущностей запроса.",started)
            except AS21SourceUnavailable:return self._source_failure(session,"source_unavailable","Источник AS21 временно недоступен. Нельзя подтвердить сущности запроса.",started)
            except AS21SourceError:return self._source_failure(session,"source_protocol_error","Источник AS21 вернул некорректные данные при проверке сущностей.",started)
        if frame.clarifications:
            pending=_PendingDialogue(frame,list(frame.clarifications)); self._pending[session]=pending; return self._clarification_response(session,pending)
        return await self._execute_frame(frame,session,started)

    @staticmethod
    def _decorate(response,llm_used):
        if response.data is None: response.data={}
        if isinstance(response.data,dict):
            meta=response.data.setdefault("_harness",{})
            if isinstance(meta,dict): meta["llm_used"]=llm_used; meta["dialogue_state"]="answered"; meta["feedback_prompt"]="Ответ помог? Что бы вы хотели улучшить?"
