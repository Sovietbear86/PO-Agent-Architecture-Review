# Assignment 042 — Readiness-Gated Oracle Recovery Retest

## Purpose

Assignment 041 was a valid BLOCKED report: GigaCode changed only the allowed
report, but could not prove a stable independent oracle path because PO Agent
and MCP-SWTR behaved inconsistently after service restart.

This assignment retests after the production readiness fix that makes root
`/health` readiness-aware instead of a process-only liveness check. The goal is
to prevent QA from starting TS acceptance while runtime/source readiness is
degraded.
...
Use `START_HEAD`, the current branch head after `git pull --ff-only`, containing
the readiness-aware root `/health` fix and this assignment.
...
Do not commit JSON, temporary scripts, runner changes, config changes, logs,
`.env`, credentials, historical reports, roadmap edits, or production changes.
...
If a helper or runner cannot parse a valid response, do not edit the helper or
runner. Record `QA_INFRA_BLOCKED` with exact raw response shape.

## Autonomous execution

The repository owner pre-authorizes this QA batch.

Do not ask for confirmation after each step, integration call, TS case, local
service restart, read-only AS21/SWTR query, MCP-SWTR diagnostic, Task API
diagnostic, HTTP diagnostic, test command, allowed report commit, or allowed
report push.

Ask only if continuing requires a missing credential, unavoidable platform
approval, write outside the report allowlist, production/source-data/config
mutation, destructive out-of-scope action, or scope expansion.
...
4. Verify `START_HEAD` contains Assignment 041 report commit:

```bash
git merge-base --is-ancestor 4ce7e595e0a6112f45cc4f3162030e0c9f065809 "$START_HEAD"
```
...
If any preflight check fails, write the allowed 042 report with
`042_VERDICT = BLOCKED`, include exact evidence, commit only that report, push,
and stop.

## Phase 1 — readiness validation

Restart services cleanly from `START_HEAD`:
...
Then validate readiness before any TS case:

1. `GET http://127.0.0.1:8004/live`
   - must return process liveness only.
2. `GET http://127.0.0.1:8004/health`
   - must include runtime/source readiness fields:
     `runtime`, `adapter`, `semantic_mode`, `source_status`, `source_facts`,
     `skill_readiness`, `runtime_init_error`.
3. `GET http://127.0.0.1:8004/api/v1/health`
   - must agree with root `/health`.

Do not run TS acceptance while root `/health` is missing readiness fields or
while `status != healthy`, `source_status != healthy`, or `runtime_init_error`
is non-null.

If readiness is degraded, write `042_VERDICT = BLOCKED` with exact health
payloads and stop.

## Phase 2 — prove independent oracle path

Prove one independent source-backed oracle path.
...
3. Task API `/api/v1/swtr-read/*` endpoints, only if used directly for source
   hydration and not derived from PO Agent query output.
...
- at least one hydrated known task unit with key, assignee, status,
  product/space and sprint attributes where present;
- explanation why this source is independent of PO Agent output.

If no independent oracle path can be proven, do not run TS cases as acceptance.
Write `042_VERDICT = BLOCKED`, include exact 403/502/error responses and manual
action required, commit only the allowed report, push, and stop.

## Phase 3 — TS-01..TS-12 exact-set rerun
...
6. Extract `AGENT_KEYS` from structured PO Agent response data.
...
Clarification is acceptable only if source evidence shows the query is genuinely
ambiguous or ungroundable and no required slot was silently dropped.
...
If any field cannot be collected, mark the row `BLOCKED` or `FAIL` with exact
reason. Do not omit rows.

## Acceptance

042 is GREEN only if all of the following are true:
...
If any condition is false, 042 is RED or BLOCKED. Do not mark GREEN.

`READY_TO_RESUME_GATE_E` must remain `NO` in this assignment. Gate E can only
resume after a later valid full rollup explicitly authorizes it.
...
ROOT_HEALTH_STATUS = healthy|degraded|other
API_V1_HEALTH_STATUS = healthy|degraded|other
